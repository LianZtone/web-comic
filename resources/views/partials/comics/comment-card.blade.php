@php
    $depth = $depth ?? 0;
    $isReply = $depth > 0;
    $deleteModalId = 'comic-delete-comment-' . $comment['id'];
...
<article @class([
    ...
    <div class="mt-4 flex flex-wrap items-center gap-2">
        @if (($commentVotesReady ?? false) === true)
            <form method="POST"
                action="{{ route('comics.comments.vote', ['slug' => $comicSlug, 'comment' => $comment['id']]) }}"
        ...
        @auth
            @if (($comment['can_manage'] ?? false) === true)
                <div class="mt-4 hidden rounded-[1.2rem] border border-base-300/60 bg-base-100/50 p-4"
                    ...
                    <form method="POST"
                        action="{{ route('comics.comments.update', ['slug' => $comicSlug, 'comment' => $comment['id']]) }}"
                        class="space-y-3">
                        @csrf
                        @method('PATCH')
            ...
            @if (($commentRepliesReady ?? false) === true)
                <div class="mt-4 hidden rounded-[1.2rem] border border-base-300/60 bg-base-100/50 p-4"
                    ...
                    <form method="POST" action="{{ route('comics.comments.store', ['slug' => $comicSlug]) }}"
                        class="space-y-3" data-reader-comment-form data-captcha-scope="series-feedback"
                        ...
                        @include('partials.forms.captcha', [
                            ...
                            'captchaQuestion' => \App\Support\FormCaptcha::question(request(), 'series-feedback'),
            ...
            <div class="modal" role="dialog">
                <div class="modal-box max-w-md rounded-[1.6rem] border border-base-300/70 bg-base-100">
                    ...
                    <div class="modal-action">
                        ...
                        <form method="POST"
                            action="{{ route('comics.comments.destroy', ['slug' => $comicSlug, 'comment' => $comment['id']]) }}">
                            @csrf
                            @method('DELETE')
